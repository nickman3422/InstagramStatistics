...
def get_keys(product_url):
    ...
    tcin=str(response.text).split("tcin")[6].split('"')[2]
    return apikey,tcin
def checkifinstock(product_urls):
    ...
    while instock is False:
        for url in product_urls:
            ...
            if( '"shipping_options":{"availability_status":"IN_STOCK"' in str(response.text) ):
                instock =True
                print("it is instock")
                return url

def addtocart(req,product_url,apikey,tcin):
...
def checkout(req,product_url,apikey,tcin):
    ...
    response = req.post("https://carts.target.com/web_checkouts/v1/pre_checkout?field_groups=ADDRESSES%2CCART%2CCART_ITEMS%2CDELIVERY_WINDOWS%2CPAYMENT_INSTRUCTIONS%2CPICKUP_INSTRUCTIONS%2CPROMOTION_CODES%2CSUMMARY&key=feaf228eb2777fd3eee0fd5192ae7107d6224b39",data=json.dumps(data),headers=headers)
    print("response: ", response)
    ...
    parmas = {'key': apikey}
    data = {"cart_id":"fc8b89a0-f840-11ea-9c14-c360e71f599b","card_number":"4767718349196032"}
    response = req.post("https://carts.target.com/checkout_payments/v1/credit_card_compare?9",data=json.dumps(data),headers=headers, parmas=parmas)
    print("response: ", response),
    print("response.txt: ", response.text)
    ...
    #parmas = {'field_groups':'ADDRESSES',2CCART,2CCART_ITEMS2CDELIVERY_WINDOWS%2CPAYMENT_INSTRUCTIONS%2CPICKUP_INSTRUCTIONS%2CPROMOTION_CODES%2CSUMMARY&key=feaf228eb2777fd3eee0fd5192ae7107d6224b39}
